...
void Kinect::updateSPoint()
{
	...
	spinePx = myDistance3d(jointA.Position, jointB.Position);
...
void Kinect::save()
{
	...
	string filePath = "images/" + currentDateTime(0) + "_" + frameCollection.getLabel() + ".txt";
...
void Kinect::drawStatusText()
{
	...
	yd = cv::getTextSize("a", fontFace, fontScale, fontThickness, nullptr).height * 1.4;
	...
	cv::putText(srcMat, "Distance : " + to_string(distance), point, fontFace, fontScale, statusFontColor, fontThickness);
	point.y += yd;
	statusStream.str("");

	cv::putText(srcMat, "Spinepx : " + to_string(spinePx), point, fontFace, fontScale, statusFontColor, fontThickness);
	point.y += yd;
#endif

#ifdef Show_Status_MLVar
	cv::putText(srcMat, "LHandActivated : " + to_string(leftHandActivated), point, fontFace, fontScale, statusFontColor, fontThickness);
	point.y += yd;
	statusStream.str("");

	cv::putText(srcMat, "RHandActivated : " + to_string(rightHandActivated), point, fontFace, fontScale, statusFontColor, fontThickness);
	point.y += yd;
	statusStream.str("");

	cv::putText(srcMat, "Recording : " + to_string(frameStacking), point, fontFace, fontScale, statusFontColor, fontThickness);
	point.y += yd;
	statusStream.str("");

	cv::putText(srcMat, "StackedCnt : " + to_string(frameCollection.getCollectionSize()), point, fontFace, fontScale, statusFontColor, fontThickness);
	point.y += yd;
	statusStream.str("");
	
	if (mode == KINECT_MODE_OUTPUT)
	{
		cv::putText(srcMat, "RecordedCnt : " + to_string(recorded), point, fontFace, fontScale, statusFontColor, fontThickness);
		point.y += yd;
		statusStream.str("");
	}
	...
	{
		cv::putText(srcMat, "LastPredicted : " + to_string(LABEL(lastPredict)), point, fontFace, fontScale, statusFontColor, fontThickness);
		point.y += yd;
		statusStream.str("");
	}
