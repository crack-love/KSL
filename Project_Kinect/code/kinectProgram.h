...
//#pragma comment(lib, "ws2_32.lib") // √Êµπ πÊ¡ˆ
//#include <winsock2.h> // windowsµÓ ±∏πˆ¿¸ «Ï¥ıøÕ √Êµπ
...
private:
	...
	int depthWidth = 512, depthHeight = 424; // kinect v2??depth ?∞Ïù¥???¨Í∏∞
...
private:
	...
	// for extract hand
	void extractHand(cv::Mat& screen);			// colorFrame?êÏÑú ?êÏùÑ tracking?òÏó¨ screen??Î≥µÏÇ¨
	void extractDepthHand(cv::Mat& screen);     // depthFrame?êÏÑú ?êÏùÑ tracking?òÏó¨ screen??Î≥µÏÇ¨
	void extractBodyIndexHand(cv::Mat& screen); // bodyIndexFrame?êÏÑú ?êÏùÑ tracking?òÏó¨ screen??Î≥µÏÇ¨
